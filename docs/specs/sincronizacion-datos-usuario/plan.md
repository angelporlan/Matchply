...
2. Añadir el cliente de la skill para enviar el paquete temporal y eliminar el token/env files siempre, incluso tras un error.
...
6. Generar un token efímero, importarlo, retirar el token mediante `env-update` y verificar salud y recuentos. El paquete local se elimina al finalizar.
