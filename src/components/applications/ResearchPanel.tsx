...
export default function ResearchPanel({ offerId, initialResearch }: { offerId: string; initialResearch: ResearchData | null }) {
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 border-b border-subtle pb-4">
        <div>
          <h3 className="text-sm font-bold text-text uppercase tracking-wider flex items-center gap-2"><Sparkles className="w-4 h-4 text-ai" /> Investigación profunda</h3>
          <p className="text-xs text-slate-500 dark:text-text-muted mt-1">Oferta, empresa, personas y señales públicas. Las fuentes se conservan como extractos limitados.</p>
        </div>
        {research && <span className={`self-start text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${research.status === 'completed' ? 'text-emerald-600 bg-emerald-500/10 border-emerald-500/20' : research.status === 'failed' ? 'text-rose-500 bg-rose-500/10 border-rose-500/20' : 'text-ai bg-ai/10 border-ai/20'}`}>{statusLabel(research.status)}</span>}
      </div>
      ...
      {!research ? (
        <div className="rounded-xl border border-dashed border-ai/30 bg-ai/5 p-8 text-center space-y-3">
          ...
          <button type="button" onClick={() => void retry()} disabled={loading} className="inline-flex items-center gap-2 bg-ai-action text-on-ai-action font-bold text-xs px-4 py-2.5 rounded-lg disabled:opacity-50"><Sparkles className="w-4 h-4" /> Iniciar investigación</button>
        </div>
      ) : (
        <>
          ...
          <div className="rounded-xl border border-subtle p-4 space-y-3">
            <div className="flex items-center justify-between"><h4 className="text-xs font-bold uppercase tracking-wider text-text">Progreso por agente</h4>{['queued', 'running'].includes(research.status) && <Loader2 className="w-4 h-4 animate-spin text-ai" />}</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{agents.map(agent => <div key={agent.id} className="flex items-center justify-between rounded-lg bg-canvas/30 p-3 text-xs"><span className="font-semibold text-text">{agent.role}</span><span className={`flex items-center gap-1 ${agent.status === 'completed' ? 'text-emerald-500' : agent.status === 'failed' ? 'text-rose-500' : 'text-ai'}`}>{agent.status === 'completed' ? <CheckCircle2 className="w-3.5 h-3.5" /> : agent.status === 'failed' ? <AlertCircle className="w-3.5 h-3.5" /> : <Clock3 className="w-3.5 h-3.5" />}{agent.status}</span></div>)}</div>
          </div>

          {report && <div className="space-y-4">
            <div className="rounded-xl bg-ai/5 border border-ai/20 p-4"><div className="text-[10px] uppercase tracking-wider font-bold text-ai mb-2">Recomendación: {report.recommendation || 'insufficient_evidence'}</div><p className="text-sm text-text leading-relaxed">{report.executiveSummary}</p></div>
            ...
            <div className="rounded-xl border border-subtle p-4"><h4 className="text-xs font-bold uppercase tracking-wider text-text">Próximos pasos</h4><ul className="mt-2 space-y-1.5 text-xs text-slate-600 dark:text-slate-300">{(report.nextSteps || []).map((item: string, index: number) => <li key={index}>• {item}</li>)}</ul></div>
          </div>}

          <div className="space-y-2"><h4 className="text-xs font-bold uppercase tracking-wider text-text">Fuentes utilizadas ({sourceList.length})</h4>{sourceList.length ? <div className="space-y-2">{sourceList.map((source: any, index: number) => <a key={`${source.url}-${index}`} href={source.url} target="_blank" rel="noopener noreferrer" className="flex items-start gap-2 p-3 rounded-lg border border-subtle hover:border-ai/40 transition-colors"><ExternalLink className="w-3.5 h-3.5 text-ai mt-0.5 shrink-0" /><span className="text-xs text-text truncate">{source.title || source.domain || source.url}</span></a>)}</div> : <p className="text-xs text-slate-500">No se han encontrado fuentes públicas accesibles.</p>}</div>
          ...
          {['failed', 'partial'].includes(research.status) && <button type="button" onClick={() => void retry()} disabled={loading} className="inline-flex items-center gap-2 border border-ai/30 text-ai font-bold text-xs px-4 py-2.5 rounded-lg disabled:opacity-50">{loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />} Reintentar investigación</button>}
        </>
      )}
    </div>
  );
